...
import edu.mirmil.web.utils.JwtProvider
...
class UserServiceImpl(private val userRepository: UserRepository) : UserService {
    ...
    override fun login(user: User): String {
        val userData = userRepository.findOne(Example.of(user)).orElseThrow {
            throw Exception("가입되지 않은 계정입니다.")
        }

        return JwtProvider.jwtCreator(userData.idx.toString(), userData.name!!)
    }
}